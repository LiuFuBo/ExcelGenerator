...
- 文件名自动追加 `.xlsx` 后缀
- 新文件会创建"销售日期"和"销售单号"两个列标题，并在下方写入对应数据
...
### 文件保存位置
- 新建模式：Excel文件保存在软件所在目录
  - Mac .app → .app所在目录（如 dist/）
  - Windows .exe → exe所在目录
  - 开发模式 → app.py所在目录
- 选择文件模式：直接保存到选中的文件路径
